            ...
            >
                ...
                <section id="leireg" className="detailSection">
                    ...
                    <div className={style.leiDetail}>
                        ...
                        <div className={style.leiDetailData}>{item.attributes.identifier}</div>
                    </div>
                    {item.attributes.lei_status ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>LEI Status</div>
                            <div className={style.leiDetailData}>{item.attributes.lei_status}</div>
                        </div>
                    ) : ''}
                    {item.attributes.lou ? (
                        <div className={style.leiDetail}>
                    ...
                    {item.attributes.first_registration ? (
                        <div className={style.leiDetail}>
                            ...
                            <div className={style.leiDetailData}>{germanDate(item.attributes.first_registration)}</div>
                        </div>
                    ) : ''}
                    {item.attributes.auto_renew == true ? (
                            <div className={style.leiDetail}>
                                <div className={style.leiDetailTitle}>Letzte Verlängerung</div>
                                <div className={style.leiDetailData}>{germanDate(auto_last_renewal)}</div>
                            </div>
                    ) : 
                    item.attributes.auto_renew !== true && item.attributes.last_renewal !== null ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Letzte Verlängerung</div>
                            <div className={style.leiDetailData}>{germanDate(item.attributes.last_renewal)}</div>
                        </div>
                    ): '' }
                    {item.attributes.auto_renew == true ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Nächste Verlängerung</div>
                            <div className={style.leiDetailData}>{germanDate(auto_next_renewal)}</div>
                        </div>
                    ) : ''}
                </section>
                    ...
                    {item.attributes.company.data !== null ? (
                            <div className={style.leiDetail}>
                                ...
                                <div className={style.leiDetailData}>
                                    <Link className="inlineLink" href={'/companies/'+item.attributes.company.data.attributes.pageslug}>                            
                        ...
                        <Alert theme="warning">
                            Mit diesem Legal Entitiy Identifier ist kein Unternehmen verbunden.
                        </Alert>
                    )}
                    {item.attributes.company.data !== null && item.attributes.company.data.attributes.legal_form !== null ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Rechtsform</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.legal_form}</div>
                        </div>
                    ) : ''}
                    {item.attributes.company.data !== null && item.attributes.company.data.attributes.hr_court !== null ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Registergericht</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.hr_court}</div>
                        </div>
                    ) : ''}
                    {item.attributes.company.data !== null && item.attributes.company.data.attributes.hr_number !== null ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Registernummer</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.hr_dept+' '+item.attributes.company.data.attributes.hr_number}</div>
                        </div>
                    ) : ''}
                    {item.attributes.company.data !== null && item.attributes.company.data.attributes.status !== null ? (
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Status</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.status}</div>
                        </div>
                    ) : ''}
                </section>
                <section id="mainbranch" className="detailSection">
                    <h4 className="mb-3 sectionLabel">Juristischer Sitz / Hauptniederlassung</h4>
                    {item.attributes.company.data.attributes.main_branch.data !== null ? (
                        <>
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>Straße, Hausnummer</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.main_branch.data.attributes.street+' '+item.attributes.company.data.attributes.main_branch.data.attributes.place_number}</div>
                        </div>
                        <div className={style.leiDetail}>
                            <div className={style.leiDetailTitle}>PLZ, Ort</div>
                            <div className={style.leiDetailData}>{item.attributes.company.data.attributes.main_branch.data.attributes.zip+' '+item.attributes.company.data.attributes.main_branch.data.attributes.city}</div>
                        </div>
                        </>
                    ) : (
                        <Alert theme="warning">
